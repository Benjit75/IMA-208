...
class KalmanFilter(object):
    ...
    def __init__(self, dim_x, dim_z):
        ...
        self.x = np.zeros((self.dim_x, 1)) # state (mean)
        self.P = np.eye(self.dim_x) # uncertainty covariance
        self.Q = np.eye(self.dim_x) # process uncertainty
        self.F = np.eye(self.dim_x) # state transition matrix
        self.H = np.eye(self.dim_x) # measurement matrix
        self.R = np.eye(self.dim_z) # measurement uncertainty
